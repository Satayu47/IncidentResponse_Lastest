"""
Measure ChatGPT (GPT-4o) Latency for Comparison
================================================

Measures end-to-end latency for ChatGPT baseline:
...
def measure_chatgpt_latency(text: str, adapter: LLMAdapter) -> Dict[str, Any]:
    """Measure ChatGPT classification latency."""
    start_time = time.time()
    
    try:
        # ChatGPT classification (end-to-end)
...
def main():
    """Measure ChatGPT latency."""
    print("=" * 70)
    print("CHATGPT LATENCY MEASUREMENT")
    print("=" * 70)
    ...
    # Check if it's a valid OpenAI key (not Anthropic)
    if openai_key.startswith("sk-or-v1-") or openai_key.startswith("sk-ant-"):
        print("[ERROR] This is an Anthropic key, not OpenAI!")
        print("OpenAI keys start with 'sk-' (not 'sk-or-v1-' or 'sk-ant-')")
        return
    ...
    try:
        adapter = LLMAdapter(model="gpt-4o", api_key=openai_key)
        print(f"\n[OK] ChatGPT (GPT-4o) initialized")
    except Exception as e:
        print(f"\n[ERROR] Failed to initialize ChatGPT: {e}")
        return
    ...
    for idx, test_case in enumerate(test_cases, 1):
        ...
        for run in range(NUM_RUNS):
            result = measure_chatgpt_latency(test_case, adapter)
            all_results.append({
                "test_case": idx,
                "run": run + 1,
                "text": test_case,
    ...
    output_file = output_dir / f"chatgpt_latency_{timestamp}.json"
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    # Print summary
    print("\n" + "=" * 70)
    print("CHATGPT LATENCY SUMMARY")
    print("=" * 70)
